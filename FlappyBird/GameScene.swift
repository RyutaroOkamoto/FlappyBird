...
//  FlappyBird
//
//  Created by RYUTARO OKAMOTO on 2019/05/11.
...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func setupGround(){
        ...
        for i in 0..<needNumber {
            ...
            // 衝突の時に動かないように設定する
            sprite.physicsBody?.isDynamic = false            // スプライトを追加する
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        if (contact.bodyA.categoryBitMask & scoreCategory) == scoreCategory || (contact.bodyB.categoryBitMask & scoreCategory) == scoreCategory {
            ...
            let sound = SKAction.playSoundFileNamed("Cursor1.mp3", waitForCompletion: false)
            run(sound)
        }else {
            // 壁か地面と衝突した
            print("GameOver")
    ...
    func setupItem(){
        ...
        let random_y_range = birdSize.height * 3
        // 下の壁のY軸下限位置(中央位置から下方向の最大振れ幅で下の壁を表示する位置)を計算
        let itemSize = SKTexture(imageNamed: "apple").size()
        ...
        let createItemAnimation = SKAction.run({
            ...
            let random_y = CGFloat.random(in: 0..<random_y_range)
            // Y軸の下限にランダムな値を足して、下の壁のY座標を決定
            let itemposi = under_wall_lowest_y + slit_length / 2 + itemTexture.size().height / 2
            
            let items = SKSpriteNode(texture: itemTexture)
            items.position = CGPoint(x: 0, y: itemposi)
            ...
            item.addChild(items)
            items.physicsBody?.contactTestBitMask = self.birdCategory
